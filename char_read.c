...
img read_img(FILE *f) { int8_t header[54];
  ...
  int8_t *dat; fread(dat,im.w*im.h*3,1,f);
  for(int i=0;i<im.w*im.h*3;i+=3) { 
    im.dat[i/3] = (col) { dat[i], dat[i+1], dat[i+2] }; }
  return im; }
...
int main(int argc,char **argv) { G_ftbl = malloc((++G_fsz)*sizeof(gra));
  ...
  FILE *g = fopen("graph.bmp","r");
  img bg = read_img(g); fclose(g); return 0; }
